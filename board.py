...
class Othello:
    def __init__(self):
        p.init()
        self.screen = p.display.set_mode((1100, 800))
...
class Grid:
    def __init__(self, rows, columns, size, main):
        ...
        self.x = columns
        self.size = size
        self.whitetoken = util.loadImages('assets/WhiteToken.png', size)
        self.blacktoken = util.loadImages('assets/BlackToken.png', size)
        self.transitionWhiteToBlack = [util.loadImages(f'assets/BlackToWhite{i}.png', self.size) for i in range(1, 4)]
        self.transitionBlackToWhite = [util.loadImages(f'assets/WhiteToBlack{i}.png', self.size) for i in range(1, 4)]
    ...
    def loadBackGroundImages(self):
        ...
        for i in range(3):
            for j in range(7):
                imageDict[alpha[j]+str(i)] = util.loadSpriteSheet(spriteSheet, j, i, (self.size), (32, 32))
        return imageDict
        ...
        for gridX, row in enumerate(grid):
            for gridY, col in enumerate(row):
                if grid[gridX][gridY] != 0:
                    continue
                DIRECTIONS = util.directions(gridX, gridY)
    ...
    def swappableTiles(self, x, y, grid, player):
        surroundCells = util.directions(x, y)
